...
for i in range(len(Ideal_massive)):
    if (i % 2 == 0):
        Ideal_type.append(Ideal_massive[i])
    elif (i % 2 == 1):
        Ideal_time.append((float(Ideal_massive[i])))
Ideal.append(Ideal_type)
Ideal.append(Ideal_time)
print(Ideal_type)
print(Ideal_time)
file.close()
...
class Animaton_person():
    ...
    def do_animation(self, type, num):
        ...
        if not self.is_scene:
            ...
            allow_timer = threading.Timer(len(self.imgs_animations[type]) * self.one_shot_time / 1000 - 200 / 1000,
        ...
        for i in range(num):
            for j in range(len(self.imgs_animations[type])):
                t = threading.Timer(
                    i * len(self.imgs_animations[type]) * self.one_shot_time / 1000 + j * self.one_shot_time / 1000,
                    draw_shot, (type, j))
                t.start()
...
class Sheep(Animaton_person):
    def __init__(self, screen, path, is_scene):
        ...
        for i in range(99):
            self.imgs_animations[0].append(
                pygame.image.load(self.path + 'd3/5011a4d85853406fe543643984c44bb7-' + str(i) + '.png'))
...
def play():
    global Player_time, Player_type
    ...
    while 1:
        ...
        text1 = f1.render(str(points), 1, (180, 0, 0))
        surf.fill((0,0,0))
        surf.blit(text1,(0,0))
        screen.screen.blit(surf,(220,50))
        pygame.display.update()


def check(a, b):
    global Ideal_time, Ideal_type
    for i in range(len(Ideal_time)):
        if b - Ideal_time[i] <= 5 and b - Ideal_time[i] >= -5 and int(a) == int(Ideal_type[i]):
            return 1
    return 0
